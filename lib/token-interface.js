...
module.exports =  {
    async init(redisInterface, web3, accountConfig, poolConfig, pool_env ) {
        ...
        this.mintHelperContract = new web3.eth.Contract(mintHelperContractJSON.abi, this.accountConfig.minting.address)
    ...
    async getPoolDifficultyTarget() {
        return targetString = await this.redisInterface.loadRedisData('miningTarget')
    },
    ...
    async getEthBlockNumber() {
        let result = parseInt(await this.redisInterface.loadRedisData('ethBlockNumber'))
    ...
    async queueMiningSolution(solution_number, minerEthAddress, challenge_digest, challenge_number) {
        const currentTokenMiningReward = await this.requestCurrentTokenMiningReward()
    ...
    async getMinerData(minerEthAddress) {
        const minerDataJSON = await this.redisInterface.findHashInRedis('miner_data', minerEthAddress)

        return JSON.parse(minerDataJSON)
    },
    //copied from peer
    async getMinerList() {
        return await this.redisInterface.getResultsOfKeyInRedis('miner_data')
    },
