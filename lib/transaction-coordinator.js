...
module.exports =  {
    ...
    */
    ...
    async getTransactionStatistics() {
        ...
        var queuedTransactions = await this.redisInterface
            .getElementsOfListInRedis('queued_transactions')

        var ethereumTransactionHashes = await this.redisInterface
            .getResultsOfKeyInRedis('active_transactions')
    ...
    async transferTokensFromPool(
        ...
        var addressFrom = this.getPaymentAccount().address
        var transferMethod = this.tokenContract.methods.transfer(
            addressTo, amount)
    ...
    async getWorkingMergeMintAccounts(
        solution_number,
        challenge_digest,
        max_gas_cost,
        addressFrom,
        ...
        for (let i = 0; i < this.mergeMintTokens.length; i++) {
    ...
    async submitMiningSolution(minerAddress,solution_number,challenge_digest,challenge_number,resending){
        ...
        try {
            var mintMethod = this.mintHelperContract.methods.proxyMergeMint(
