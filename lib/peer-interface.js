...
module.exports = {
    ...
    */
    async handlePeerShareSubmit(
        ...
        const poolChallengeNumber = await this.tokenInterface.getPoolChallengeNumber()
        const computed_digest = web3utils.soliditySha3( poolChallengeNumber , poolHelperEthAddress, nonce )

        const digestBytes32 = web3utils.hexToBytes(computed_digest)
        const digestBigNumber = web3utils.toBN(computed_digest)

        const minShareTarget = web3utils.toBN(this.getPoolMinimumShareTarget() )
        const miningTarget = web3utils.toBN(await this.tokenInterface.getPoolDifficultyTarget() )
    ...
    async handleValidShare(
        ...
        if (existingShare === null) {
            ...
            const challengeNumber = await this.tokenInterface.getPoolChallengeNumber()
    ...
    async processQueuedShares() {
        ...
        const shareDataJSON = await this.redisInterface.popFromRedisList(
            'queued_shares_list')
    ...
    async monitorMinedSolutions() {
        ...
        try {
            ...
            const solution_txes = await this.redisInterface.getResultsOfKeyInRedis(
    ...
    async requestTransactionReceipt(tx_hash) {
        /* Initilialize receipt. */
        let receipt = null

        try {
            receipt = await this.web3.eth.getTransactionReceipt(tx_hash)
        } catch (err) {
    ...
    async initJSONRPCServer(port) {
        ...
        var server = jayson.server({
            ...
            getChallengeNumber: async function(args, callback) {
                console.log('GET CHALLENGE NUMBER', args, callback)
                var challengeNumber = await self.redisInterface.loadRedisData('challengeNumber' )
            ...
            getAllMiningParameters: async function(args, callback) {
                ...
                if ("challengeNumber" in args) {
                    let challengeNumber = await self.redisInterface.loadRedisData('challengeNumber');
            ...
            submitShare: async function (args, callback) {
                ...
                const poolChallengeNumber = await self.tokenInterface.getPoolChallengeNumber()
